...
void TpccGenerator::generateCustomerAndHistory()
{
   ...
   array<char, 30> c_since = {}; // XXX used in history and customer and generated over and over again
   array<char, 2> c_credit = {};
   int64_t c_credit_lim;
   float c_discount;
   float c_balance;
   array<char, 50> c_data = {};
   float h_amount;
   ...
   for (c_w_id = 1; c_w_id<=warehouse_count; c_w_id++) {
      for (c_d_id = 1; c_d_id<=kDistrictsPerWarehouse; c_d_id++) {
         for (c_id = 1; c_id<=kCustomerPerDistrict; c_id++) {
            ...
            makeAlphaString(30, 50, c_data.data());
...
void TpccGenerator::generateOrdersAndOrderLines()
{
   ...
   int64_t o_ol_cnt;
   array<char, 19> o_entry_d = {}; // XXX not sure if date is generate correctly
   ...
   csv::CsvWriter o_csv(folder + "/orders.csv");
   o_csv << csv::endl;
   csv::CsvWriter ol_csv(folder + "/order_line.csv");
   ol_csv << csv::endl;
   csv::CsvWriter no_csv(folder + "/new_orders.csv");
   no_csv << csv::endl;
   ...
   for (o_w_id = 1L; o_w_id<=warehouse_count; o_w_id++) {
      for (o_d_id = 1L; o_d_id<=kDistrictsPerWarehouse; o_d_id++) {
         ...
         for (o_id = 1; o_id<=OrdersPerDistrict; o_id++) {
            ...
            // @formatter:off
            o_csv << o_id << o_d_id << o_w_id << o_c_id << o_entry_d << o_carrier_id
            ...
            for (ol_number = 1; ol_number<=o_ol_cnt; ol_number++) {
               ...
               if (o_id>2100) {
                  ...
                  // @formatter:off
                  ol_csv << o_id << o_d_id << o_w_id << ol_number << ol_i_id << o_w_id << o_entry_d
                  ...
                  ol_csv << o_id << o_d_id << o_w_id << ol_number << ol_i_id << o_w_id
                         << o_entry_d << ol_quantity << csv::Precision(2) << ol_amount << ol_dist_info << csv::endl;
                  // @formatter:on
               }
            }
