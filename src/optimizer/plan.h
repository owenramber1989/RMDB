...
{
    public:
        SortPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<std::shared_ptr<ast::OrderBy>> orders,int limit)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
            orders_ = orders;
...
class AggrePlan : public Plan
{
public:
    AggrePlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::shared_ptr<ast::AggreClause> aggreClause){
        Plan::tag = tag;
        subplan_ = std::move(subplan);
