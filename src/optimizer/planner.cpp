...
// TODO(ZMY) 修改匹配规则为最左匹配，支持单点查询和范围查询
bool Planner::get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<std::string>& index_col_names) {
    index_col_names.clear();
    // acquire col_name
    std::vector<std::string> check_list;
    for(auto& cond:curr_conds){
        if(cond.is_rhs_val&&(cond.op==OP_EQ || cond.op==OP_GT) && cond.lhs_col.tab_name==tab_name){
            check_list.push_back(cond.lhs_col.col_name);
        }
    }
    TabMeta& tab = sm_manager_->db_.get_table(tab_name);
    IndexMeta index_to_use;
    // 优先使用完全匹配
    for (auto& index:tab.indexes) {
        std::vector<std::string> col_names;
//        if (index.col_num!=check_list.size())continue;
        auto& index_cols=index.cols;
        // exist表示完全匹配是否存在
        bool exist=true;
        for(auto &col:index_cols){
            if (std::find_if(check_list.begin(),check_list.end(),[&](std::string& name){return name==col.name;})==check_list.end()){
                exist= false;
                break;
            }
            else{
        ...
        if (exist&&index.col_num==check_list.size()){
            index_col_names=col_names;
            return true;
        }
        // match 表示该索引是否最左匹配
        bool match= true;
        ...
        if(match){
            if (col_names.size()>index_col_names.size()){
                index_col_names=col_names;
                index_to_use=index;
            }
        }
    ...
    if (!index_col_names.empty()){
        index_col_names.clear();
        for(auto& col:index_to_use.cols){
...
std::shared_ptr<Plan> Planner::physical_optimization(std::shared_ptr<Query> query, Context *context)
{
    ...
    if(x->has_aggre) {
        plan = generate_aggre_plan(query, std::move(plan));
    }
    if(x->has_sort) {
        // 处理orderby
...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    ...
    if(x->has_limit) {
        return std::make_shared<SortPlan>(T_Sort,std::move(plan),x->orders,x->limit->number_);
    }
    return std::make_shared<SortPlan>(T_Sort, std::move(plan), x->orders, -1);
}


std::shared_ptr<Plan> Planner::generate_aggre_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    auto x = std::dynamic_pointer_cast<ast::SelectStmt>(query->parse);
    ...
    return std::make_shared<AggrePlan>(T_Aggre, std::move(plan), std::move(x->aggreClause) );
}
/**
