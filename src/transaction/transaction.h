...
public:
  explicit Transaction(txn_id_t txn_id, IsolationLevel isolation_level =
      ...
      : state_(TransactionState::DEFAULT), isolation_level_(isolation_level),
        txn_id_(txn_id) {
  ...
  inline std::shared_ptr<std::deque<WriteRecord *>> get_write_set() {
    ...
    inline std::shared_ptr<std::deque<IndexWriteRecord *>> get_index_write_set() {
        return index_write_set_;
    }
  inline void append_write_record(WriteRecord *write_record) {
    write_set_->push_back(write_record);
  }
    inline void append_index_write_record(IndexWriteRecord *index_write_record) {
      index_write_set_->push_back(index_write_record);
  }
    bool exists_in_write_set(const WriteRecord& target) const {
        return std::find_if(write_set_->begin(), write_set_->end(), [&target](const WriteRecord* record){
            return *record == target;
        }) != write_set_->end();
    }
    void remove_from_write_set(const WriteRecord& target) {
        auto end = std::remove_if(write_set_->begin(), write_set_->end(), [&target](const WriteRecord* record){
            return *record == target;
        });
        write_set_->erase(end, write_set_->end());
    }
...
private:
  ...
  std::mutex latch_; // 锁住这个事务，避免死锁检测和释放锁修改事务状态发生冲突
};
